        ...
        >
          ...
          >
            <template slot-scope="{ row }">
              <span v-if="row.dispatchStatus==='1'">待调度</span>
              <span v-if="row.dispatchStatus==='2'">未匹配线路</span>
              <span v-if="row.dispatchStatus==='3'">已调度</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="orderNumber"
          ...
          >
            <template slot-scope="{ row }">
              <base-button
                ...
                @click="handleDetails(row.waybillNumber)"
              >查看详情</base-button>
            </template>
          </el-table-column>
        </el-table>
...
import { getWayList } from '@/module-waybill/api/index'
import { waybillStatus } from '@/utils/common-select-data'
...
export default class extends mixins(SinglePage) {
  ...
  private async getList() {
    // this.loading = true
    const data = (await getWayList(this.search)) as any
    if (data) {
      this.loading = false
