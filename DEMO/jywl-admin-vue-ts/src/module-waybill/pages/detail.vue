...
// 基础数据
// api数据接口
import { getListDetails, getListTrack } from '@/module-waybill/api/index'
...
export default class extends Vue {
  ...
  private points = [
    {
      name: '出发点',
      icon: require('../../assets/images/iconstar.png'),
      markerPoint: {
        lng: 0,
        lat: 0
      }
    ...
    {
      name: '终点',
      icon: require('../../assets/images/iconend.png'),
      markerPoint: {
        lng: 0,
        lat: 0
      }
  ...
  // 获取运单轨迹
  async getOrderLocusParams(id: any) {
    const data = (await getListTrack(id)) as any
    this.centerList = data.items // 获取路线点
    this.content = data.licensePlate
    this.polylinePath.push(data.vehicleInformation) // 获取车辆当前位置
    this.points[0].markerPoint = {
      lng: this.centerList[0].lng,
      lat: this.centerList[0].lat
    }
    this.points[1].markerPoint = {
      lng: this.centerList[this.centerList.length - 1].lng,
      lat: this.centerList[this.centerList.length - 1].lat
    }
  }
